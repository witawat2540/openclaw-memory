#!/bin/bash
# Build Observer v18 - cyclecount_mobile
...
echo "$(date): Build Observer v18 (cyclecount_mobile) started"
...
while true; do
    ...
    # --- Android Detection ---
    # ตรวจจับเฉพาะหน้าแก้ไขของ appid=183
    ...
    # --- Web Detection ---
    # เช็คที่ version app (cycle-count.gbhcenter.com)
    if [[ "$CURRENT_URL" == *"cycle-count.gbhcenter.com"* ]]; then
    ...
    if $SEEN_ANDROID || $SEEN_WEB || $SEEN_IOS; then
        ...
        for i in {1..300}; do
            ...
            if [[ "$CHECK_URL" == *"cycle-count.gbhcenter.com"* ]] && [[ "$CHECK_URL" == *"version"* ]]; then
                SEEN_WEB=true
            fi
        ...
        if $SEEN_ANDROID && ! $SEEN_WEB; then
            PENDING="$PENDING [Web: cycle-count.gbhcenter.com/version]"
        fi
        ...
        if [ -n "$PENDING" ]; then
            MSG="🚀 เจ้านายครับ! ตรวจพบการ Upload โปรเจกต์ CycleCount แล้ว แต่ $PENDING ยังไม่ได้ upload นะครับ! 🦾"
